...
static bool allocate_distance_map_tests() {
...

  {
    ...
    size_t invokations = 0;
    auto map = CpuDistanceMapTestAccess::Create(cpu_count, [&invokations](cpu_num_t, cpu_num_t) {
      invokations++;
      return 1u;
    });
    EXPECT_FALSE(map.has_value());
    EXPECT_EQ(0u, invokations);
  }

  {
    ...
    EXPECT_EQ(entry_count, invokations);
  }
