...
// 1GB pages
const uintptr_t l1_large_page_size = 1UL << (PAGE_SIZE_SHIFT + 2 * RISCV_MMU_PT_SHIFT);
const uintptr_t l1_large_page_size_mask = l1_large_page_size - 1;

// 2MB pages
const uintptr_t l2_large_page_size = 1UL << (PAGE_SIZE_SHIFT + RISCV_MMU_PT_SHIFT);
const uintptr_t l2_large_page_size_mask = l2_large_page_size - 1;

static size_t vaddr_to_l0_index(uintptr_t addr) {
  // canonicalize the address
  addr &= RISCV_MMU_CANONICAL_MASK;
  return ((addr >> PAGE_SIZE_SHIFT) >> (3 * RISCV_MMU_PT_SHIFT)) & (RISCV_MMU_PT_ENTRIES - 1);
}

static size_t vaddr_to_l1_index(uintptr_t addr) {
  // canonicalize the address
  addr &= RISCV_MMU_CANONICAL_MASK;
  return ((addr >> PAGE_SIZE_SHIFT) >> (2 * RISCV_MMU_PT_SHIFT)) & (RISCV_MMU_PT_ENTRIES - 1);
}

static size_t vaddr_to_l2_index(uintptr_t addr) {
  // canonicalize the address
  addr &= RISCV_MMU_CANONICAL_MASK;
  return ((addr >> PAGE_SIZE_SHIFT) >> RISCV_MMU_PT_SHIFT) & (RISCV_MMU_PT_ENTRIES - 1);
}

static size_t vaddr_to_l3_index(uintptr_t addr) {
  // canonicalize the address
  addr &= RISCV_MMU_CANONICAL_MASK;
  return (addr >> PAGE_SIZE_SHIFT) & (RISCV_MMU_PT_ENTRIES - 1);
}
...
extern "C" /*zx_status_t*/ size_t riscv64_boot_map(pte_t* kernel_table0, const vaddr_t vaddr,
  ...
  while (off < len) {
    ...
    if (!(kernel_table0[index0] & RISCV_PTE_V)) { // invalid/unused entry
      paddr_t pa = boot_alloc_ptable();
      kernel_table0[index0] = RISCV_PTE_PPN_TO_PTE(pa) | RISCV_PTE_V;
      kernel_table1 = reinterpret_cast<pte_t*>(pa);
    } else {
      if (!(kernel_table0[index0] & RISCV_PTE_PERM_MASK)) {
        kernel_table1 = reinterpret_cast<pte_t*>(RISCV_PTE_PPN(kernel_table0[index0]));
      } else {
    ...
    if (!(kernel_table1[index1] & RISCV_PTE_V)) { // invalid/unused entry
      if ((((vaddr + off) & l1_large_page_size_mask) == 0) &&
          (((paddr + off) & l1_large_page_size_mask) == 0) && (len - off) >= l1_large_page_size) {
        kernel_table1[index1] = RISCV_PTE_PPN_TO_PTE(((paddr + off) & ~l1_large_page_size_mask)) | flags;
      ...
      paddr_t pa = boot_alloc_ptable();
      kernel_table1[index1] = RISCV_PTE_PPN_TO_PTE(pa) | RISCV_PTE_V;
      kernel_table2 = reinterpret_cast<pte_t*>(pa);
    } else {
      if (!(kernel_table1[index1] & RISCV_PTE_PERM_MASK)) {
        kernel_table2 = reinterpret_cast<pte_t*>(RISCV_PTE_PPN(kernel_table1[index1]));
      } else {
        // not legal to have a block pointer at this level
        return ZX_ERR_BAD_STATE;
      }
    ...
    if (!(kernel_table2[index2] & RISCV_PTE_V)) { // invalid/unused entry
      if ((((vaddr + off) & l2_large_page_size_mask) == 0) &&
          (((paddr + off) & l2_large_page_size_mask) == 0) && (len - off) >= l2_large_page_size) {
        kernel_table2[index2] = RISCV_PTE_PPN_TO_PTE(((paddr + off) & ~l2_large_page_size_mask)) | flags;
      ...
      paddr_t pa = boot_alloc_ptable();
      kernel_table2[index2] = RISCV_PTE_PPN_TO_PTE(pa) | RISCV_PTE_V;
      kernel_table3 = reinterpret_cast<pte_t*>(pa);
    } else {
      if (!(kernel_table2[index2] & RISCV_PTE_PERM_MASK)) {
        kernel_table3 = reinterpret_cast<pte_t*>(RISCV_PTE_PPN(kernel_table2[index2]));
      } else {
        // not legal to have a block pointer at this level
        return ZX_ERR_BAD_STATE;
      }
    ...
    kernel_table3[index3] = RISCV_PTE_PPN_TO_PTE(paddr + off) | flags;
