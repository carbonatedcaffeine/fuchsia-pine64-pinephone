...
pte_t riscv64_kernel_translation_table[RISCV_MMU_PT_ENTRIES] __ALIGNED(PAGE_SIZE);
...
static pte_t mmu_flags_to_pte_attr(uint flags) {
  ...
  attr |= (flags & ARCH_MMU_FLAG_PERM_USER) ? RISCV_PTE_U : 0;
  attr |= (flags & ARCH_MMU_FLAG_PERM_READ) ? RISCV_PTE_R : 0;
  attr |= /*(flags & ARCH_MMU_FLAG_PERM_WRITE) ?*/ RISCV_PTE_W /*: 0*/;
  attr |= (flags & ARCH_MMU_FLAG_PERM_EXECUTE) ? RISCV_PTE_X : 0;

  return attr;
}

uint RiscvArchVmAspace::MmuFlagsFromPte(pte_t pte) {
  uint mmu_flags = 0;
  mmu_flags |= (pte & RISCV_PTE_U) ? ARCH_MMU_FLAG_PERM_USER : 0;
  mmu_flags |= (pte & RISCV_PTE_R) ? ARCH_MMU_FLAG_PERM_READ : 0;
  mmu_flags |= (pte & RISCV_PTE_W) ? ARCH_MMU_FLAG_PERM_WRITE : 0;
  mmu_flags |= (pte & RISCV_PTE_X) ? ARCH_MMU_FLAG_PERM_EXECUTE : 0;
  return mmu_flags;
}
...
zx_status_t RiscvArchVmAspace::QueryLocked(vaddr_t vaddr, paddr_t* paddr, uint* mmu_flags) {
  ulong index;
...
zx_status_t RiscvArchVmAspace::AllocPageTable(paddr_t* paddrp) {
...
void RiscvArchVmAspace::FreePageTable(void* vaddr, paddr_t paddr) {
...
volatile pte_t* RiscvArchVmAspace::GetPageTable(vaddr_t pt_index,
                                                volatile pte_t* page_table) {
  pte_t pte;
  paddr_t paddr;
  void* vaddr;
  ...
  if (!(pte & RISCV_PTE_V)) {
    ...
    pte = RISCV_PTE_PPN_TO_PTE(paddr) | RISCV_PTE_V;
    page_table[pt_index] = pte;
    LTRACEF("pte %p[%#" PRIxPTR "] = %#" PRIx64 "\n", page_table, pt_index, pte);
    return static_cast<volatile pte_t*>(vaddr);
  } else if (!(pte & RISCV_PTE_PERM_MASK)) {
    paddr = RISCV_PTE_PPN(pte);
...
zx_status_t RiscvArchVmAspace::SplitLargePage(vaddr_t vaddr, uint level,
  ...
  const auto attrs = pte & RISCV_PTE_PERM_MASK;

  const size_t next_size = page_size_per_level(level - 1);
  for (uint64_t i = 0, mapped_paddr = RISCV_PTE_PPN(pte);
       i < RISCV_MMU_PT_ENTRIES; i++, mapped_paddr += next_size) {
    new_page_table[i] = RISCV_PTE_PPN_TO_PTE(mapped_paddr) | attrs;
  }

  // Ensure all zeroing becomes visible prior to page table installation.
  mb();

  page_table[pt_index] = RISCV_PTE_PPN_TO_PTE(paddr);
...
void RiscvArchVmAspace::FlushTLBEntry(vaddr_t vaddr, bool terminal) {
...
ssize_t RiscvArchVmAspace::UnmapPageTable(vaddr_t vaddr, vaddr_t vaddr_rel, size_t size,
  ...
  while (size) {
    ...
    if (level > 0 && !(pte & RISCV_PTE_PERM_MASK)) {
      page_table_paddr = RISCV_PTE_PPN(pte);
...
// NOTE: caller must DSB afterwards to ensure TLB entries are flushed
ssize_t RiscvArchVmAspace::MapPageTable(vaddr_t vaddr_in, vaddr_t vaddr_rel_in, paddr_t paddr_in,
  ...
  while (size) {
    ...
    if (((vaddr_rel | paddr) & block_mask) || (chunk_size != block_size)) {
      ...
      pte = RISCV_PTE_PPN_TO_PTE(paddr) | attrs;
...
// NOTE: caller must DSB afterwards to ensure TLB entries are flushed
zx_status_t RiscvArchVmAspace::ProtectPageTable(vaddr_t vaddr_in, vaddr_t vaddr_rel_in,
                                                size_t size_in, pte_t attrs, uint level,
                                                volatile pte_t* page_table) {
  volatile pte_t* next_page_table;
  vaddr_t index;
  ...
  while (size) {
    ...
    if (level > 0 && pte & RISCV_PTE_PERM_MASK && chunk_size != block_size) {
    ...
    if (level > 0 && !(pte & RISCV_PTE_PERM_MASK)) {
      ...
      pte = (pte & ~RISCV_PTE_PERM_MASK) | attrs;
      LTRACEF("pte %p[%#" PRIxPTR "] = %#" PRIx64 "\n", page_table, index, pte);
      page_table[index] = pte;
...
bool RiscvArchVmAspace::HarvestAccessedPageTable(vaddr_t vaddr, vaddr_t vaddr_rel_in, size_t size,
  ...
  while (size) {
    ...
    if (level > 0 && (pte & RISCV_PTE_PERM_MASK) && chunk_size != block_size) {
    ...
    } else if (level > 0 && !(pte & RISCV_PTE_PERM_MASK)) {
      const paddr_t page_table_paddr = RISCV_PTE_PPN(pte);
      volatile pte_t* next_page_table =
      ...
      if (pte & RISCV_PTE_A) {
        const paddr_t pte_addr = RISCV_PTE_PPN(pte);
...
void RiscvArchVmAspace::MarkAccessedPageTable(vaddr_t vaddr, vaddr_t vaddr_rel_in, size_t size,
                                              uint level, volatile pte_t* page_table) {
  const vaddr_t block_size = page_size_per_level(level);
  const vaddr_t block_mask = block_size - 1;
...
ssize_t RiscvArchVmAspace::MapPages(vaddr_t vaddr, paddr_t paddr, size_t size, pte_t attrs) {
...
ssize_t RiscvArchVmAspace::UnmapPages(vaddr_t vaddr, size_t size) {
...
zx_status_t RiscvArchVmAspace::ProtectPages(vaddr_t vaddr, size_t size, pte_t attrs) {
...
void RiscvArchVmAspace::MmuParamsFromFlags(uint mmu_flags, pte_t* attrs) {
  if (attrs) {
...
zx_status_t RiscvArchVmAspace::HarvestAccessed(vaddr_t vaddr, size_t count,
  ...
  // synchronize with.
  if (HarvestAccessedPageTable(vaddr, vaddr, size, RISCV_MMU_PT_LEVELS - 1, tt_virt_,
                               accessed_callback)) {
    mb();
  }
...
zx_status_t RiscvArchVmAspace::MarkAccessed(vaddr_t vaddr, size_t count) {
  ...
  MarkAccessedPageTable(vaddr, vaddr, size, RISCV_MMU_PT_LEVELS - 1, tt_virt_);
...
zx_status_t RiscvArchVmAspace::Init(vaddr_t base, size_t size, uint flags, page_alloc_fn_t paf) {
  ...
  if (flags & ARCH_ASPACE_FLAG_KERNEL) {
    ...
    memcpy((void*)(tt_virt_ + RISCV_MMU_PT_ENTRIES/2),
	   (void*)(riscv64_get_kernel_ptable() + RISCV_MMU_PT_ENTRIES/2),
	   PAGE_SIZE/2);
  }
  pt_pages_ = 1;
...
// TODO(revest): Set both satp and xstatus to the addres space + privilege level
  if (aspace) {
    aspace->canary_.Assert();
    DEBUG_ASSERT((aspace->flags_ & (ARCH_ASPACE_FLAG_KERNEL | ARCH_ASPACE_FLAG_GUEST)) == 0);

    uint64_t satp = ((uint64_t)RISCV_SATP_MODE_SV48 << RISCV_SATP_MODE_SHIFT) |
         	    ((uint64_t)aspace->asid_ << RISCV_SATP_ASID_SHIFT) |
