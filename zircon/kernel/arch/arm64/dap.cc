...
void cpu_debug_command(cpu_num_t cpu) {
  ...
  auto dap_debug_thread = [](void *arg) -> int {
    ...
    auto ac = fbl::MakeAutoCall([]() { Thread::Current::preemption_state().PreemptReenable(); });
  ...
  auto thread = Thread::Create("dap debug", dap_debug_thread, (void *)(uintptr_t)cpu, DEFAULT_PRIORITY);
