...
class RelaxedAtomic {
 public:
  ...
  template <typename U>
  T operator+=(U&& value) {
    return fetch_add(ktl::forward<U>(value));
  }

  operator T() const noexcept { return load(); }

  T load() const { return wrapped_.load(ktl::memory_order_relaxed); }
  ...
  template <typename U>
  T fetch_add(U&& value) {
    return wrapped_.fetch_add(std::forward<U>(value), ktl::memory_order_relaxed);
  }
