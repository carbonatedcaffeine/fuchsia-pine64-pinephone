...
class Scheduler {
 ...
 private:
  ...
  // logical cluster index share the innermost cache with a significant
  // performance impact with respect to load balancing (i.e. running on other
  // processors in the same cluster has the least impact on cache affinity).
  size_t cluster_{0};
