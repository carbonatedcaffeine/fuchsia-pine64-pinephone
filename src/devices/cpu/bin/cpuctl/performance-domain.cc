...
std::variant<zx_status_t, CpuPerformanceDomain> CpuPerformanceDomain::CreateFromPath(
  ...
  auto cpu_client = std::make_unique<cpuctrl::Device::SyncClient>(std::move(cpu_local));
  auto device_client =
      std::make_unique<fuchsia_device::Controller::SyncClient>(std::move(device_local));
...
std::pair<zx_status_t, uint64_t> CpuPerformanceDomain::GetNumLogicalCores() {
  ...
  return std::make_pair(resp.status(), resp.value().count);
}
