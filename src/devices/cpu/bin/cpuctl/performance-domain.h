...
class CpuPerformanceDomain {
 ...
 private:
  ...
  explicit CpuPerformanceDomain(
      std::unique_ptr<cpuctrl::Device::SyncClient> cpu_client,
  ...
  std::unique_ptr<cpuctrl::Device::SyncClient> cpu_client_;
  std::unique_ptr<fuchsia_device::Controller::SyncClient> device_client_;
