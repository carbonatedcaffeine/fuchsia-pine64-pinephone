...
class TestCpuPerformanceDomain : public CpuPerformanceDomain {
 public:
  ...
  TestCpuPerformanceDomain(std::unique_ptr<cpuctrl::Device::SyncClient> cpu_client,
                           std::unique_ptr<fuchsia_device::Controller::SyncClient> device_client)
...
void PerformanceDomainTest::SetUp() {
  ...
  auto cpu_client = std::make_unique<cpuctrl::Device::SyncClient>(std::move(cpu_client_channel));
  auto device_client =
      std::make_unique<fuchsia_device::Controller::SyncClient>(std::move(device_client_channel));
