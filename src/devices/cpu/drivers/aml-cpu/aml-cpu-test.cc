...
TEST(AmlCpuTest, CreateTest) {
  constexpr amlogic_cpu::operating_point_t kUnsortedOperatingPoints[] = {
    { .freq_hz = 10, .volt_uv = 10, .pd_id = 0 },
    { .freq_hz = 20, .volt_uv = 20, .pd_id = 0 },
    { .freq_hz = 5, .volt_uv = 5, .pd_id = 0 },
  };

  fake_ddk::Bind ddk;

  clock_protocol_t dummy_clk;
  power_protocol_t dummy_pwr;

  ddk.SetMetadata(&kUnsortedOperatingPoints, sizeof(kUnsortedOperatingPoints));
}
