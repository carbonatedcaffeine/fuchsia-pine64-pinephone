...
constexpr size_t kPowerFragmentOffset = 0;
constexpr size_t kPllDiv16ClkFragmentOffset = 1;
constexpr size_t kCpuDiv16ClkFragmentOffset = 2;
constexpr size_t kCpuScalerClkFragmentOffset = 3;
...
zx_status_t AmlCpu::Create(void* context, zx_device_t* parent) {
  ...
  for (size_t i = 0; i < num_perf_domains; i++) {
    ...
    const size_t power_fragment_offset = (kFragmentsPerPfDomain * i + 1) + kPowerFragmentOffset;
    const size_t pll_div16_clk_fragment_offset =
        (kFragmentsPerPfDomain * i + 1) + kPllDiv16ClkFragmentOffset;
    const size_t cpu_div16_clk_fragment_offset =
        (kFragmentsPerPfDomain * i + 1) + kCpuDiv16ClkFragmentOffset;
    const size_t cpu_scaler_clk_fragment_offset =
        (kFragmentsPerPfDomain * i + 1) + kCpuScalerClkFragmentOffset;
