...
static const std::vector<uint32_t> supported_n_channels = {2};
static const std::vector<sample_format_t> supported_sample_formats = {SAMPLE_FORMAT_PCM_SIGNED};
static const std::vector<justify_format_t> supported_justify_formats = {JUSTIFY_FORMAT_JUSTIFY_I2S};
static const std::vector<uint32_t> supported_rates = {48'000, 96'000};
static const std::vector<uint8_t> supported_bits_per_channel = {32};
static const std::vector<uint8_t> supported_bits_per_sample = {16};
static const audio::DaiSupportedFormats kSupportedDaiFormats = {
    .number_of_channels = supported_n_channels,
    .sample_formats = supported_sample_formats,
    .justify_formats = supported_justify_formats,
    .frame_rates = supported_rates,
    .bits_per_channel = supported_bits_per_channel,
...
zx::status<DriverIds> Tas27xx::Initialize() {
  ...
  if (status != ZX_OK) {
    zxlogf(ERROR, "tas27xx: Could not SW reset and Init %d\n", status);
  ...
  return zx::ok(DriverIds{
      ...
      .device_name = "Device-TAS2770",
  });
}

zx_status_t Tas27xx::SoftwareResetAndInitialize() {
  auto status = SWReset();  // software reset, will be in software shutdown state after call
  if (status != ZX_OK) {
    return status;
  }
  ...
  status = SetRate(supported_rates[0]);
  if (status != ZX_OK) {
    return status;
  }
...
zx_status_t Tas27xx::Reset() { return SoftwareResetAndInitialize(); }

Info Tas27xx::GetInfo() {
  Info info = {};
  info.unique_id = "";
  info.manufacturer = "Texas Instruments";
  info.product_name = "TAS2770";
  return info;
}
